...
def plot_feature_maps(feature_maps: torch.Tensor, num_maps: int = 16,
                      title: str = "Feature Maps", figsize: Tuple[int, int] = (15, 8)):
    """
...
def plot_filters(model: nn.Module, layer_name: str = None, num_filters: int = 16,
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=figsize)
    fig.suptitle(f'Learned Filters - {conv_layer.__class__.__name__}',
...
def visualize_predictions(images: torch.Tensor, predictions: List, true_labels: List,
                         class_names: List[str] = None, num_images: int = 10,
                         figsize: Tuple[int, int] = (15, 8)):
    """
