...
# Sidebar Navigation
with st.sidebar:
    st.image("https://via.placeholder.com/150x50/1E88E5/FFFFFF?text=Finance+Tracker", use_container_width=True)
    st.markdown("## Navigation")

    page = st.radio(
        "Go to:",
        ["📊 Dashboard", "➕ Add Transaction", "💵 Income", "💸 Expenses",
         "📈 Analytics", "🎯 Budgets", "⚙️ Settings"],
    ...
    total_expenses = st.session_state.expenses['amount'].sum() if len(st.session_state.expenses) > 0 else 0
    total_income = st.session_state.income['amount'].sum() if len(st.session_state.income) > 0 else 0
...
if page == "📊 Dashboard":
    ...
    if len(st.session_state.expenses) > 0:
        ...
        with col2:
            ...
            if len(st.session_state.income) > 0:
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(x=inc_by_month.index, y=inc_by_month.values, name='Income', marker_color='green'))
                fig.add_trace(go.Bar(x=exp_by_month.index, y=exp_by_month.values, name='Expenses', marker_color='red'))
        ...
        if len(recent_expenses) > 0:
            st.dataframe(recent_expenses[['date', 'category', 'amount', 'description']], use_container_width=True)
    else:
        ...
        with st.form("expense_form"):
            ...
            with col1:
                ...
                exp_category = st.selectbox("Category",
                    ["Food", "Transport", "Entertainment", "Shopping", "Utilities", "Healthcare", "Other"])
        ...
        with st.form("income_form"):
            ...
            with col1:
                ...
                inc_category = st.selectbox("Category",
                    ["Salary", "Freelance", "Investment", "Business", "Gift", "Other"], key="inc_cat")

            with col2:
                inc_amount = st.number_input("Amount ($)", min_value=0.01, value=100.00, step=0.01, key="inc_amt")
    ...
    if len(st.session_state.expenses) > 0:
        ...
        category_summary = filtered_df.groupby('category')['amount'].agg(['sum', 'count', 'mean']).round(2)
    ...
    if len(st.session_state.income) > 0:
        ...
        # Display table
        st.dataframe(st.session_state.income.sort_values('date', ascending=False), use_container_width=True)

        # Category breakdown
        st.subheader("Income by Source")
        income_summary = st.session_state.income.groupby('category')['amount'].agg(['sum', 'count', 'mean']).round(2)
    ...
    if len(st.session_state.expenses) > 0:
        ...
        with tab1:
            ...
            dow_spending = df_dow.groupby('day_of_week')['amount'].sum().reindex(
        ...
        with tab2:
            ...
            for idx, row in top_days.iterrows():
                st.write(f"- ${row['amount']:.2f} on {row['date'].strftime('%Y-%m-%d')}: {row['description']} ({row['category']})")

            # Average by category
            st.write("\n**Average Spending by Category:**")
            avg_by_cat = st.session_state.expenses.groupby('category')['amount'].mean().sort_values(ascending=False)
        ...
        with tab3:
            ...
            if len(monthly_totals) >= 3:
                ...
                st.write(f"**Trend:** Spending is {trend} by approximately ${abs(model.coef_[0]):.2f} per month")
            else:
    ...
    with tab1:
        ...
        if uploaded_file:
            try:
                ...
                st.session_state.expenses = pd.concat([st.session_state.expenses, df_import], ignore_index=True)
                save_data()
