...
def build_phase2_notebook():
    ...
    cells.append(create_cell("markdown", """# Module 13: This, That, These, Those - Controlled Practice
...
print("[OK] Ready to practice! Complete all exercises below.")""", f"cell-{cell_counter}"))
    cell_counter += 1
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set1:
        cells.append(create_cell("code", f"""# Exercise 1.{exercises_set1.index((prompt, answer)) + 1}
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set2:
        cells.append(create_cell("code", f"""# Exercise 2.{exercises_set2.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set3:
        cells.append(create_cell("code", f"""# Exercise 3.{exercises_set3.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set4:
        cells.append(create_cell("code", f"""# Exercise 4.{exercises_set4.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set5:
        cells.append(create_cell("code", f"""# Exercise 5.{exercises_set5.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set6:
        cells.append(create_cell("code", f"""# Exercise 6.{exercises_set6.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set7:
        cells.append(create_cell("code", f"""# Exercise 7.{exercises_set7.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    exercises_set8 = [
        ...
        ("5. Incorrect: This morning (yesterday) was cold. → Correct: ___ morning was cold.", "That"),
    ...
    for prompt, answer in exercises_set8:
        cells.append(create_cell("code", f"""# Exercise 8.{exercises_set8.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
---""", f"cell-{cell_counter}"))
    ...
    mc_exercises = [
        ("___ book in my hand is interesting.", {'A': 'This', 'B': 'That', 'C': 'These'}, "A", "'This' is correct for a singular object near you."),
        ("___ cars over there are expensive.", {'A': 'This', 'B': 'That', 'C': 'Those'}, "C", "'Those' is correct for plural objects far away."),
        ("___ is my friend Sarah. (introducing someone next to you)", {'A': 'This', 'B': 'That', 'C': 'These'}, "A", "'This' is used for introductions."),
        ("___ were happy days. (talking about the past)", {'A': 'This', 'B': 'That', 'C': 'Those'}, "C", "'Those' refers to plural past times."),
        ("Hello, ___ is John speaking. (on the phone)", {'A': 'this', 'B': 'that', 'C': 'I'}, "A", "Use 'this' to identify yourself on the phone."),
        ("___ week has been very busy. (current week)", {'A': 'This', 'B': 'That', 'C': 'These'}, "A", "'This' refers to the current time period."),
        ("___ shoes I'm wearing are comfortable.", {'A': 'This', 'B': 'These', 'C': 'Those'}, "B", "'These' is correct for plural items you're wearing."),
        ("___ mountain in the distance is beautiful.", {'A': 'This', 'B': 'These', 'C': 'That'}, "C", "'That' is correct for a singular object far away."),
    ]

    for i, (question, options, correct, feedback) in enumerate(mc_exercises, 1):
        cells.append(create_cell("code", f"""# Exercise 9.{i}
...
---""", f"cell-{cell_counter}"))
    ...
    for prompt, answer in exercises_set10:
        cells.append(create_cell("code", f"""# Exercise 10.{exercises_set10.index((prompt, answer)) + 1}
exercise.create_fill_in_blank(
    "{prompt}",
    "{answer}"
)""", f"cell-{cell_counter}"))
...
## 📊 Your Score""", f"cell-{cell_counter}"))
    cell_counter += 1

    cells.append(create_cell("code", """# Display final score
validator.display_score()""", f"cell-{cell_counter}"))
    cell_counter += 1
...
**Great work!** 🌟 You've completed the controlled practice phase!""", f"cell-{cell_counter}"))
    cell_counter += 1
...
def main():
    ...
    if 25 <= file_size_kb <= 30:
        ...
        print(f"Warning: File size is outside target range. Target: 25-30 KB, Actual: {file_size_kb:.2f} KB")
