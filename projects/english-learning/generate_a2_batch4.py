...
COMPARISON_CHARACTERISTICS = {
    "formation_rules": {
        ...
        "long_adjectives": {
            ...
            "examples": ["expensive → more expensive → most expensive", "beautiful → more beautiful → most beautiful"]
        }
    },
    "irregular_forms": {
    ...
    "common_errors": [
        ...
        {"wrong": "more taller", "correct": "taller", "rule": "Short adjectives use -er, not 'more'"},
        {"wrong": "She is taller her brother", "correct": "She is taller than her brother", "rule": "Use 'than' with comparatives"},
        {"wrong": "He is tallest", "correct": "He is the tallest", "rule": "Superlatives need 'the'"}
    ]
}
...
BATCH_4_MODULES = {
    ...
    "17": {
        ...
        "key_patterns": ["the + adjective + -est", "the most + adjective", "one of the + superlative"]
    },
    "18": {
    ...
    "20": {
        ...
        "key_patterns": ["very/really/quite + adj/adv", "too + adj", "adj + enough", "enough + noun"]
    }
}
...
**Focus:** {', '.join(module_data['key_patterns'])}"""
        ...
        overview = f"""8 real-world communicative tasks focusing on {module_data['comparison_focus']}"""
...
def create_exercise_set(set_num, set_name, count, description, comparison_focus):
    ...
    cells.append(create_markdown_cell(f"""## Exercise Set {set_num}: {set_name} ({count} exercises)
...
def create_comparison_activity(activity_num, activity_name, comparison_focus):
    ...
    cells.append(create_markdown_cell(f"""## Activity {activity_num}: {activity_name}
...
def create_comparison_task(task_num, task_name, comparison_focus):
    ...
    cells.append(create_markdown_cell(f"""## Task {task_num}: {task_name}
...
def create_phase_1(module_num, module_data):
    ...
    for i, title in enumerate(section_titles[:module_data['sections']], 1):
        cells.extend(create_comparison_section_placeholder(i, title, module_data['comparison_focus']))
...
def main():
    ...
    print("- Progressive complexity (comparatives → superlatives → as...as → adverbs → intensifiers)")
    print("\nNext Steps:")
