...
INTEGRATION_MATRIX = {
    ...
    "batch_2_future": {
        "modules": [6, 7, 8, 9, 10],
        "key_structures": ["going to", "will", "present continuous for future", "time clauses", "for/since"],
...
def create_phase_1(module_num, module_data):
    ...
    for i, section_title in enumerate(section_titles, 1):
        section_cells = create_section_placeholder(i, section_title, module_data["grammar_focus"], module_num)
...
"""
    ...
    exercise_sets = [
        ("Formation Basics", exercises_per_set, f"Basic {module_data['grammar_focus'].replace('_', ' ')} formation"),
        ...
        ("Mixed Practice", module_data['exercises'] - (exercises_per_set * 9), "Comprehensive practice")
    ]

    for i, (set_name, count, description) in enumerate(exercise_sets, 1):
        set_cells = create_exercise_set(i, set_name, count, description, module_data['grammar_focus'])
...
"""
    ...
    if next_module in BATCH_5_MODULES:
        completion += f"Continue to **Module {next_module}: {BATCH_5_MODULES[next_module]['title']}**\n"
    else:
...
def main():
    ...
    for module_num in sorted(BATCH_5_MODULES.keys()):
        ...
        print(f"  Sections: {module_data['sections']} | Exercises: {module_data['exercises']} | Activities: {module_data['activities']} | Tasks: {module_data['tasks']}")
