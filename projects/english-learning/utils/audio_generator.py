...
class AudioGenerator:
    ...
    def generate_audio(self, text: str, accent: str = "us",
    ...
    def play_audio(self, text: str, accent: str = "us",
                  slow: bool = False, autoplay: bool = False):
        """
        ...
        if audio_file and os.path.exists(audio_file):
            ...
            display(HTML(f'<p style="color: #1976d2; font-style: italic;">🔊 "{text}"{speed_label}</p>'))
    ...
    def create_pronunciation_card(self, word: str, phonetic: str = "",
    ...
    def create_sentence_audio(self, sentence: str, accent: str = "us",
                             label: str = "Listen to the sentence:"):
        """
        ...
        display(HTML(f'<p style="font-size: 16px; padding: 10px; background-color: #e3f2fd; border-left: 4px solid #1976d2;">"{sentence}"</p>'))

        audio_file = self.generate_audio(sentence, accent, slow=False)
        if audio_file and os.path.exists(audio_file):
            display(Audio(audio_file))

    def create_conversation_audio(self, conversation: List[tuple],
                                 accent: str = "us"):
        """
        ...
        for speaker, text in conversation:
            ...
            display(HTML(f'<p style="margin: 10px 0;"><strong style="color: #1976d2;">{speaker}:</strong> {text}</p>'))
    ...
    def batch_generate(self, texts: List[str], accent: str = "us",
...
def create_audio_player_with_transcript(audio_file: str, transcript: str):
    ...
    if os.path.exists(audio_file):
        display(HTML(f'<p style="background-color: #e8f5e9; padding: 10px; border-radius: 5px;"><strong>Transcript:</strong> {transcript}</p>'))
        display(Audio(audio_file))
    else:
