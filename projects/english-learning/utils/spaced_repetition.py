...
class SpacedRepetitionScheduler:
    ...
    def mark_module_complete(self, module_id: str, module_name: str,
                            score: float = 100.0):
        """
    ...
    def display_review_dashboard(self):
        ...
        if due:
            ...
            display(HTML(df_due.to_html(index=False, escape=False,
        ...
        if upcoming:
            ...
            display(HTML(df_upcoming.to_html(index=False, escape=False,
                                             classes='table table-striped',
                                             border=0)))
    ...
    def export_progress_report(self) -> pd.DataFrame:
        ...
        for module_id, data in self.progress.items():
            report_data.append({
                ...
                'Avg Score': f"{sum(data['scores']) / len(data['scores']):.1f}%" if data['scores'] else "N/A",
                'Next Review': data['next_review'],
                'Status': 'Due' if data['next_review'] and datetime.fromisoformat(data['next_review']).date() <= datetime.now().date() else 'Scheduled'
            })
